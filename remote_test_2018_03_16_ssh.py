...
def ssh_send_cmd_switch( background_run,  PROGRAM_PATH , STRESS_CMD , LOG_SAVE ):
     if(DEBUG_OS_TYPE == os_linux):
          if(background_run == background_run_enable):
                      os.system( LINUX_BACKGROUND_RUN + 'ssh ' + SSH_IGNORE_HOST_KEY + os_user + '@' + os_ip_addr +' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD + ' &')
          elif(background_run == background_run_disable):
               if(LOG_SAVE == 1):
                      os.system('ssh '+ SSH_IGNORE_HOST_KEY + os_user + '@'+ os_ip_addr + ' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD + ' > ' + SSH_LOG_PATH_LINUX)
               else:
                      os.system('ssh '+ SSH_IGNORE_HOST_KEY + os_user + '@'+ os_ip_addr + ' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD)
          else:
               DEBUG('ssh_send_cmd_switch : ERROR!!  Incorrect type of background_run !!')
               return ERROR               
     elif(DEBUG_OS_TYPE == os_win):
          if(background_run == background_run_enable):
                      os.system( WIN_BACKGROUND_RUN + WIN_SSH_PATH + SSH_IGNORE_HOST_KEY + os_user + '@' + os_ip_addr +' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD + ' &')
          elif(background_run == background_run_disable):
               if(LOG_SAVE == 1):
                      os.system(WIN_SSH_PATH + SSH_IGNORE_HOST_KEY + os_user + '@'+ os_ip_addr + ' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD + ' > ' + SSH_LOG_PATH_WIN)                 
               else:
                      os.system(WIN_SSH_PATH + SSH_IGNORE_HOST_KEY + os_user + '@'+ os_ip_addr + ' -t sudo ' + PROGRAM_PATH + ' ' + STRESS_CMD)         
          else:
               DEBUG('ssh_send_cmd_switch : ERROR!!  Incorrect type of background_run !!')
               return ERROR            
     else:
...
def system_config_detect_py(ipmi):
     ...
     if(pch_tem == ERROR):
          print(system_config_detect_py.__name__ + ':Sensor number: %2x not exist' %pch_tem)
     ...
     if(cups_core_workload == ERROR):
          print(system_config_detect_py.__name__ + ': CUPS core Workload Sensor number: %2s not exist' %cups_core_workload)
    ...
    # NM_TEST_EN[ 9]  = DISABLE
     ...
     if(hpio_pwr_reading == ERROR):
          print(system_config_detect_py.__name__ + ': HPIO Domain PWR Sensor number: %2s not exist' %hpio_domain_pwr)
          ...
          for loop in range(179, 187):
               ...
               if(hpio_pwr_reading != ERROR):
                    print(system_config_detect_py.__name__ + ': HPIO PWR dedicated sensor number: %2x exist' %loop)
                    print(system_config_detect_py.__name__ + ': HPIO snnsor number : 0x%2x reading = %2d' %(loop, hpio_pwr_reading))
...
def run_schedule_py(ipmi, test_schedule):
     if os.path.isfile(TEST_RESULT_WIN) :
          # Delete previous test result
          os.remove(TEST_RESULT_WIN)
     # Create New Test Result file
     file = open(TEST_RESULT_WIN, 'w')
     ...
     for test_item in test_schedule:
          ...
          DEBUG('Save test Result in file.. ')
          file = open(TEST_RESULT_WIN, 'a')
          if(NM_TEST_STS[test_item] == SUCCESSFUL):
               print(NM_TEST_ITEM[test_item] + ' Test : Pass ')
               file.write(NM_TEST_ITEM[test_item] + ' Test : Pass\n')
          elif(NM_TEST_STS[test_item] == NONE):
               print( NM_TEST_ITEM[test_item] + ' Not in test list ,  Not Test')
               file.write(NM_TEST_ITEM[test_item] + ' Not in test list ,  Not Test\n')
          else:
               print(NM_TEST_ITEM[test_item] + ' Test: Fail !!!')
               print('Please enable debug mode for more detail test information')
               file.write(NM_TEST_ITEM[test_item] + '  Test: Fail !!!\n')
          file.close()          
...
def NM_009_WIN(ipmi):
     ...
     if(power_average_stress == ERROR):
         print(NM_009_WIN.__name__ + ':Platform power reading error!!!')
         return ERROR
     # Power Reading OK
     print(NM_009_WIN.__name__ + ':Platform Power Reading OK')
     print(NM_009_WIN.__name__ + ':Currently Full Stress Average Platform Power Reading = %6d watts' %power_average_stress)
     ...
     if(max_draw_range > (0.8*power_average_stress) and (0.8*power_average_stress) > min_draw_range):
         # Power Draw Range OK
         print(NM_009_WIN.__name__ + ':Platform Power Draw Range Setting OK')
         print(NM_009_WIN.__name__ + ":Power Capping value = %6d watt" % (power_average_stress*4/5))
     else:
         print(NM_009_WIN.__name__ + ':Error ! Platform Power Draw Rnage Setting Error!!!')
         return ERROR
     # Check if Correction time data are correct
     if(min_correction_time == ERROR):
         print(NM_009_WIN.__name__ + ':Error! Correction Time value not correct !!!')
         return ERROR
     # Correction Time OK
     print(NM_009_WIN.__name__ + ':Correction Time Value OK')
     print(NM_009_WIN.__name__ + ":Set correction time value = %6d msec" % (min_correction_time))
     ...
     if(sts == ERROR):
         print(NM_009_WIN.__name__ + ':set_nm_power_policy fail !!!')
         return ERROR
     ...
     # Below is check the power reading value to make sure if power capping really work. Make sure power drop to capping value.
         ...
         print(NM_009_WIN.__name__ + ':Power limit error!!! Platfrom power reading still higher than capping value !!!')
         print(NM_009_WIN.__name__ + ':Expected limit value = %6d , But currecnt platfrom power reading %6d' %((power_average_stress*4/5) , power_average_cap))
         return ERROR
     # Power Reading OK
     print(NM_009_WIN.__name__  + ':Platform Power Reading OK')
     print(NM_009_WIN.__name__  + ':After NM Capping average platform power reading = %6d watts' %power_average_cap)
...
def NM_010_WIN(ipmi):
     ...
     if(power_average_stress == ERROR):
         print(NM_010_WIN.__name__ + ':CPU power reading error!!!')
         return ERROR
     # Power Reading OK
     print(NM_010_WIN.__name__ + ':CPU Power Reading OK')
     print(NM_010_WIN.__name__ + ':Currently Full Stress Average CPU Power Reading = %6d watts' %power_average_stress)
     # Read Power Draw Range and Correction time via 0xC9h cmd
     ...
     if(max_draw_range > (0.8*power_average_stress) and (0.8*power_average_stress) > min_draw_range):
         # Power Draw Range OK
         print(NM_010_WIN.__name__ + ':CPU Power Draw Range Setting OK')
         print(NM_010_WIN.__name__ + ":Power Capping value = %6d watt" % (power_average_stress*4/5))
     else:
         print(NM_010_WIN.__name__ + ':Error ! CPU Power Draw Rnage Setting Error!!!')
         return ERROR
     # Check if Correction time data are correct
     if(min_correction_time == ERROR):
         print(NM_010_WIN.__name__ + ':Error! Correction Time value not correct !!!')
         return ERROR
     # Correction Time OK
     print(NM_010_WIN.__name__ + ':Correction Time Value OK')
     print(NM_010_WIN.__name__ + ":Set correction time value = %6d msec" % (min_correction_time))
     # Set Power Capping via 0xC1h cmd in police id #3, target limit = 80% of full stress value, correction time = minimum support correction time
     ...
     if(sts == ERROR):
         print(NM_010_WIN.__name__ + ':set_nm_power_policy fail !!!')
         return ERROR
     ...
     # Below is check the power reading value to make sure if power capping really work. Make sure power drop to capping value.
         ...
         print(NM_010_WIN.__name__ + ':Power limit error!!! CPU power reading still higher than capping value !!!')
         print(NM_010_WIN.__name__ + ':Expected limit value = %6d , But currecnt CPU power reading %6d' %((power_average_stress*4/5) , power_average_cap))
         return ERROR
     # Power Reading OK
     print(NM_010_WIN.__name__  + ':CPU Power Reading OK')
     print(NM_010_WIN.__name__  + ':After NM Capping average platform power reading = %6d watts' %power_average_cap)
...
def RESET_CYCLE(ipmi):
     ...
     if(OS_STS == 1):
         ...
         sts = ssh_send_cmd_switch(background_run_enable,  SSH_CMD_PATH_EMPTY , centos_reset, LOG_SAVE_OFF )
         if(sts != SUCCESSFUL):
             print('RESET_CYCLE: Reset system ERROR !!')
             return ERROR 
         print(RESET_CYCLE.__name__ + ':RESET OS, Waiting for OS boot up again....')
...
def RESET_CYCLE_LOOP_WIN(ipmi , loop):
     ...
     for count in range(0, loop):
         ...
         # Reset 
         sts = RESET_CYCLE(ipmi)
         # Check if System Reset and BIOS Post          
         if(sts == ERROR):
             print(DC_CYCLE_LOOP_WIN.__name__ + ':RESET CYCLE LOOP TEST FAIL !!!')
             return ERROR
         # Check if System Reset and BIOS Post Normally
         eop = 1
         time_count = 0
         while(eop == 1):
             rsp, current_status, operation_state , eop= mesdc_get_version_py(ipmi)
             if(rsp == ERROR):
                 print(RESET_CYCLE_LOOP_WIN.__name__ + ':RESET CYCLE TEST FAIL !!!')
                 return ERROR
             if(time_count > 50): # Make sure program will not go to dead loop due to system error status
                 print(RESET_CYCLE_LOOP_WIN.__name__ + ': RESET CYCLE TEST FAIL !!!, SYSTEM CAN NOT RESET NORMALLY')
                 return ERROR             
             time_count = time_count + 1
             time.sleep(1)
         # Check ME status make sure ME FW in normal operation state
         rsp, current_status, operation_state, eop = mesdc_get_version_py(ipmi)
         if(current_status !=5):
              print(RESET_CYCLE_LOOP_WIN.__name__ + ': ME FW not in normal operation state. Reset CYCLE LOOP TEST FAIL !!!')
              return ERROR
...
def test_enviornment_initial_check(ipmi):
     ...
     #ssh.connect(os_ip_addr,  22, os_user, "abcdef")
     #ssh.close()
     ...
     if(timeout == OS_BOOT_FAIL_TIMEOUT ):
          ...
          return ERROR         
     #Initial OS PTU
     PTU_INITIAL_SCRIPT = ptu_initial_script_detect()
